...
BLOG_AUTHOR = "Ben Landrum"  # (translatable)
...
BLOG_EMAIL = "8enlandrum@gmail.com"
BLOG_DESCRIPTION = "A PhD student at Cornell working on vector search"  # (translatable)

PUBLICATIONS_FOLDER = "publications"  # specify the folder where .yaml files are stored
publications = []

for filename in os.listdir(PUBLICATIONS_FOLDER):
    if filename.endswith('.yaml'):
        with open(os.path.join(PUBLICATIONS_FOLDER, filename), 'r') as f:
            publication = yaml.safe_load(f)
...
})(window,document,'script','dataLayer','GTM-NNPDQP26');</script>
...
# (translatable)
BODY_END = """<!-- Google Tag Manager (noscript) -->
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-NNPDQP26"
